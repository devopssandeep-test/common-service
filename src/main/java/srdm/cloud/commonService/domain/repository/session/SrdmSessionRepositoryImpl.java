...
@Repository
public class SrdmSessionRepositoryImpl implements InitializingBean, SrdmSessionRepository {

	private static final Logger logger = LoggerFactory.getLogger(SrdmSessionRepositoryImpl.class);
	...
	public String addSession(UserDetail userDetail) {
		...
		sessionDto.setSessionTimeout(userDetail.getSessionTimeout());
		...
		logger.info("addSession(): sessionId=[" + userDetail.getSessionId()
			+ "] domainId=[" + userDetail.getDomainId()
			+ "] accountId=[" + userDetail.getAccountId()
			+ "] groupId=[" + userDetail.getGroupId()
			+ "] timeout=" + userDetail.getSessionTimeout());
		return sessionId;
	}
	...
	public boolean invalidateSession(String sessionId) {
		...
		SessionMap sessionMap = SessionMap.getInstance();
		boolean bResult = sessionMap.invalidate(sessionId);
		if (bResult == true) {
			logger.info("invalidateSession(): sessionId=[%s] removed.", sessionId);
		} else {
			logger.info("invalidateSession(): sessionId=[%s] not found.", sessionId);
		}
		return bResult;
	}
